...
Scene* Level3::createScene(int num[4][3],int level)
{
...
void Level3::addBall(int num[4][3]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 3; j++) {
			levelball[i][j] = num[i][j];
		}
	}
	nr = 5 + num[2][0];  ng = 20 + num[2][1];  ny = 5 + num[2][2];
...
bool Level3::init()
{
	...
	_listener1->onTouchBegan = CC_CALLBACK_2(Level3::onTouchBegan, this);		//加入觸碰開始事件
	_listener1->onTouchMoved = CC_CALLBACK_2(Level3::onTouchMoved, this);		//加入觸碰移動事件
	_listener1->onTouchEnded = CC_CALLBACK_2(Level3::onTouchEnded, this);		//加入觸碰離開事件
...
void Level3::doStep(float dt)
{
	...
	if (_bboxR && _bboxG && _bboxB) {
		...
		for (int i = 0; i < 3; i++) {
			CCString* s = CCString::createWithFormat("+%d", levelball[3][i]);
...
bool Level3::onTouchBegan(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)//觸碰開始事件
{
...
void  Level3::onTouchMoved(cocos2d::Touch *pTouch, cocos2d::Event *pEvent) //觸碰移動事件
{
	Point touchLoc = pTouch->getLocation();
	if (startGame) {
		if (_bTouchOn)
...
void  Level3::onTouchEnded(cocos2d::Touch *pTouch, cocos2d::Event *pEvent) //觸碰結束事件 
{
	Point touchLoc = pTouch->getLocation();
	if (_bTouchOn)
	{
